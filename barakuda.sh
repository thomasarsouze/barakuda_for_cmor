...
[ -z ${CDFTOOLS_ROOT+x} ] && export CDFTOOLS_ROOT=${PWD}
...
while getopts C:R:f:y:z:c:FeEh option ; do
    case $option in
        ...
        z) export YEARN=${OPTARG} ; export LFORCE_YEND=true ;;
...
# If CMOR files, then overides a few functions with
if [ ${CMOR} -eq 1 ]; then
    echo "Sourcing ${BARAKUDA_ROOT}/src/bash/bash_functions_cmor.bash"
    . ${BARAKUDA_ROOT}/src/bash/bash_functions_cmor.bash
fi
#
...
if ${LFORCE_YEND}; then
    if [ ${jyear} -gt ${YEARN} ]; then lcontinue=false; fi
fi
...
##!!TA rm -rf ${TMP_DIR} 2>/dev/null ; #debug
