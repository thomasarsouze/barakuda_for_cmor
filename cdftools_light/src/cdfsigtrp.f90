PROGRAM cdfsigtrp
  ...
  DO jt = 1, nt !LB
     ...
     CALL CHECK_4_MISS(cfilet, trim(cv_t), lmv, missing_value)
     CALL GETVAR_3D(idf_t, idv_t,  cfilet, trim(cv_t),     nt, jt, T_3D)
     IF (lmv) THEN
       WHERE(T_3D == missing_value) T_3D = 0.0
     ENDIF
     CALL CHECK_4_MISS(cfilet, trim(cv_s), lmv, missing_value)
     CALL GETVAR_3D(idf_s, idv_s,  cfilet, trim(cv_s),     nt, jt, S_3D)
     IF (lmv) THEN
       WHERE(S_3D == missing_value) S_3D = 0.0
     ENDIF
     CALL CHECK_4_MISS(cfileu, trim(cv_u), lmv, missing_value)
     CALL GETVAR_3D(idf_u, idv_u,  cfileu, trim(cv_u),     nt, jt, U_3D)
     IF (lmv) THEN
       WHERE(U_3D == missing_value) U_3D = 0.0
     ENDIF
     CALL CHECK_4_MISS(cfilev, trim(cv_v), lmv, missing_value)
     ...
     DO jsec=1,nsection
        ...
        IF (imin == imax ) THEN        ! meridional section
           l_merid=.TRUE.
           npts=jmax-jmin
           PRINT *,' Section ',TRIM(csection(jsec)),' is meridional'
        ELSE IF ( jmin == jmax ) THEN  ! zonal section
           npts=imax-imin
           PRINT *,' Section ',TRIM(csection(jsec)),' is zonal'
        ELSE
